...
print('1',skin_df.head())
skin_df["path"] = skin_df["image_id"].map(imageid_path_dict.get) # map image_id to the path of that image
print('2',skin_df.head())
...
#print(skin_df.head())
skin_df["cell_type_idx"] = pd.Categorical(skin_df["cell_type"])
print('kk',skin_df["cell_type_idx"])
skin_df["cell_type_idx"] = pd.Categorical(skin_df["cell_type"]).codes # give each cell type a category id
print('pp',skin_df["cell_type_idx"])
#print(skin_df[["dx","cell_type_idx"]].sample(20))
...
'''
print('man')
print(skin_df['path'],skin_df["cell_type_idx"],skin_df["cell_type_idx"].values)
reshaped_image = skin_df["path"].map(lambda x: np.asarray(Image.open(x).resize((224,224), resample=Image.LANCZOS).\
                                                          convert("RGB")))
...
X_train_orig, X_test, y_train_orig, y_test = train_test_split(out_vec, labels, test_size=0.1, stratify=labels)

np.save("D:/AI in medicine/final_project_mobilenet/mob_data/224_224_test.npy", X_test)
np.save("D:/AI in medicine/final_project_mobilenet/mob_data/test_labels.npy", y_test)


X_train, X_val, y_train, y_val = train_test_split(X_train_orig, y_train_orig, test_size=0.1111,  stratify=y_train_orig)

np.save("D:/AI in medicine/final_project_mobilenet/mob_data/224_224_val.npy", X_val)
np.save("D:/AI in medicine/final_project_mobilenet/mob_data/val_labels.npy", y_val)

np.save("D:/AI in medicine/final_project_mobilenet/mob_data/224_224_train.npy", X_train)
np.save("D:/AI in medicine/final_project_mobilenet/mob_data/train_labels.npy", y_train)
