...
for layer in model.layers[1:3]:
    layer.trainable = Conv2D(64, (3, 3), dilation_rate=(1,1))
for layer in model.layers[3:4]:
    layer.trainable = True
for layer in model.layers[4:6]:
    layer.trainable = Conv2D(128, (3, 3), dilation_rate=(2,2))
for layer in model.layers[6:7]:
    layer.trainable = True
for layer in model.layers[7:11]:
    layer.trainable = Conv2D(256, (3, 3), dilation_rate=(4,4))
for layer in model.layers[11:12]:
    layer.trainable = True
for layer in model.layers[12:16]:
    layer.trainable = Conv2D(512, (3, 3), dilation_rate=(8,8))
for layer in model.layers[16:17]:
    layer.trainable = True
for layer in model.layers[17:21]:
    layer.trainable = Conv2D(512, (3, 3), dilation_rate=(16,16))
...
save_best = ModelCheckpoint(filepath='D:/AI in medicine/final_project_mobilenet/vgg19/checkpoint-{val_acc:.4f}.h5',monitor='val_acc',mode='auto',save_best_only=True)
